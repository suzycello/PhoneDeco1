...
const modelUtils = (schema) => {
  const validator = schema.reduce(
    (
      ...
      acc[key] = (value) => {
        if (!value && !optional) throw new Error(`${key} is required`);
        if (value && typeof value !== type)
          throw new TypeError(
            `Expected ${key} to be ${type} but got ${typeof value}`,
          );
        if (value && regexp && !value.match(regexp))
          throw new Error(`${value} not valid for ${key}. ${regexpMessage}`);
        if (value && otherCondition && !otherCondition(val))
          throw new Error(
